...
@push('script')
    <script>
        $(document).on('click', '.btn-edit', function () {
            ...
            $('#editModal form').attr('action', '{{ route('lainnya.bagian.update', '') }}/' + id);
            $('#editModal input:hidden#id').val(id);
            $('#editModal input#nama_bagian').val($(this).data('nama_bagian'));
...
@section('content')
                            ...
                            {{-- <td><span
                             ...
                             <td>
                                <button class="btn btn-info btn-sm btn-edit"
                                        ...
                                        data-nama_bagian="{{ $bagian->nama_bagian }}"
                                        data-bs-toggle="modal"
                                        data-bs-target="#editModal">
                                    {{ __('menu.general.edit') }}
                                </button>
                                <form action="{{ route('lainnya.bagian.destroy', $bagian->id_bagian) }}" class="d-inline" method="post">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="modal fade" id="createModal" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog">
            <form class="modal-content" method="post" action="{{ route('lainnya.bagian.store') }}">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="createModalTitle">{{ __('Tambah Bagian') }}</h5>
                    <button
                        type="button"
                        class="btn-close"
                ...
                <div class="modal-body">
                    <x-input-form name="nama_bagian" :label="__('Nama Bagian')"/>
                </div>
